...
// Prefer recommendation-provided live sources; fallback to static mapping.
function sourcesForCareer(career) {
  if (Array.isArray(career?.sources) && career.sources.length) return career.sources;
...
function ResultsViewV2({ recommendations, profile, onStartOver, onBackToConversation, sidebarVariant, density = 'standard' }) {
  ...
  return (
    <div className="mirror-results-root" style={{
      background: '#F8FBFF',
      color: '#141C21',
      height: '100vh',
      ...
      <style>{`
        @media print {
          ...
          .mirror-results-root,
          .mirror-results-shell,
          .mirror-results-scroll {
            height: auto !important;
          ...
          .mirror-results-root [aria-hidden="true"],
          .no-print,
          .mirror-career-modal {
          ...
          .mirror-results-root article {
            break-inside: avoid;
            page-break-inside: avoid;
          }

          .mirror-results-root h2,
          .mirror-results-root h3 {
            break-after: avoid;
            page-break-after: avoid;
          }

          .mirror-results-root section {
            break-before: auto;
      ...
      <div className="mirror-results-shell" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      {/* Header */}
      <header className="no-print" style={{
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <button onClick={onStartOver} style={{ ...btnV2, background: 'oklch(0.22 0.015 240)', color: '#fff' }}>Start over</button>
        </div>
      </header>

      <div className="mirror-results-scroll" style={{ flex: 1, overflowY: 'auto', scrollBehavior: 'smooth' }}>
      {/* Hero synthesis */}
      <section style={{
...
function CareerCardV2({ career, index, density, onOpen }) {
  ...
  return (
    <article
      ...
      style={{
        background: hover
          ? 'linear-gradient(160deg, oklch(0.99 0.008 240 / 0.54), oklch(0.95 0.02 245 / 0.36))'
          : 'linear-gradient(160deg, oklch(0.99 0.008 240 / 0.48), oklch(0.96 0.015 245 / 0.30))',
        border: '1px solid transparent',
        borderRadius: 20,
        padding: compact ? '22px 24px 20px' : '28px 28px 24px',
        display: 'flex', flexDirection: 'column',
        cursor: 'pointer',
        transition: 'box-shadow 240ms, transform 240ms, background 240ms',
        boxShadow: hover
          ? 'inset 0 1px 0 rgba(255,255,255,0.58), inset 0 -1px 0 rgba(255,255,255,0.14), 0 10px 24px rgba(22,40,66,0.10), 0 20px 44px rgba(22,40,66,0.05)'
      ...
      <div style={{
        ...
        <button
          ...
          {sourcesOpen && <SourcesPopover sources={sourcesForCareer(career)} />}
        </button>
      </div>
...
function CareerDetailModal({ career, index, onClose }) {
  ...
  return (
    <div
      className="mirror-career-modal no-print"
      onClick={onClose}
      style={{
...
function SourcesPopover({ sources }) {
  return (
    ...
    >
      ...
      {sources.map((entry, i) => {
        const s = typeof entry === 'string' ? SOURCE_LIBRARY[entry] : entry;
        if (!s) return null;
        const sid = typeof entry === 'string' ? entry : entry.id || `source-${i}`;
        const ts = TIER_STYLES[s.tier] || TIER_STYLES.structured;
        return (
          <div key={sid} style={{
            padding: '10px 0',
            ...
            <div style={{
              ...
              <span style={{
                ...
                {s.url ? <a href={s.url} target="_blank" rel="noreferrer" style={{ color: 'inherit', textDecoration: 'underline' }}>{s.label}</a> : s.label}
              </span>
              <span style={{
